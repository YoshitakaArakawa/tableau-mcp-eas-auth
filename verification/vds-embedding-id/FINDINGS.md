...
## ホスト経由の E2E(20260804 追記)

`query-workbook-datasource` 実装のデプロイ後、ChatGPT のカスタムコネクタ経由で
本番経路を実走し、チェーン全体の成立を確認した。

1. `render-interactive-viz` で対象ビューを MCP App iframe に描画
2. 「このビューの裏にある全量データから集計して」という自然文の依頼だけで、
   モデルは**誘導なしに** `query-workbook-datasource` を選択(スナップショットの
   `datasources[].id` + `vds` セッション2値をそのまま引数に使用)
3. クエリ成功。リーグごとの総得点上位5件を返し、さらに preamble の照合指示に
   従って**画面上の50行(5リーグ×10シーズン)との一致を自発的に検証**、
   フィルタ状態(チーム全選択)にも言及した

補足: コネクタのツールリスト更新は、ホスト側のコネクタ設定にある
再接続/ツールリスト更新の操作が必要だった(サーバーの再デプロイだけでは
ホストのツール一覧は更新されない)。claude.ai ホストは iframe 描画の
CSP 制約(viz-state 検証時に記録済み)により E2E 未実施。

### 全シート拡張後の E2E(20260804 追記)

datasources をダッシュボード配下の全ワークシート和集合に拡張したビルドで、
2 シート・2 データソース構成のダッシュボードを対象に再実行した。

- スナップショットには**両シートのデータソースが `worksheets` ラベル・
  `isPublished` 付きで載った**(ホスト側モデルに逐語ダンプさせて確認。
  以前の E2E で `isPublished` が無いように見えたのはモデルの要約脱落で、
  実データには含まれていた)
- 「第2シート(サンプルデータ無し)の裏の全量データを集計して」という依頼に対し、
  モデルは**和集合から第2シートのデータソースを正しく選択**した。しかも同名候補との
  違い(もう一方は未決済ポジション込みのキャッシュフロー)を理由として説明した —
  `worksheets` ラベルが選択根拠として機能している
- この回のクエリ経路は `query-datasource`(published を LUID 解決する代替経路)だった。
  push 文言は isPublished: true にそれを許しており、意図どおりの分岐。セッション経路
  (`query-workbook-datasource`)は前回までの E2E で実証済み
