...
### 12. ステートレス運用(DISABLE_SESSION_MANAGEMENT=true)でのクリーン一周 — 全項目成立

サーバー側のインメモリセッション消失(マシン自動停止のたびに `no valid session ID` や
ウィジェット無応答が発生していた)への対策として `DISABLE_SESSION_MANAGEMENT=true` を
適用した構成で、埋め込み→州選択→フルデータ分析の一周を実測:

1. 埋め込み描画・embed トークン取得・分析クエリまで全経路がセッション無しで成立した。
   マシン再起動をまたいだセッション切れという故障クラスは構造的に消滅
2. 状態同期: Texas 選択と期間フィルターがスナップショット経由でクエリ条件に反映された
3. **照合が自発的にフル実行された**。KPI の一致(再計算 -15.118% = 画面表示)に加え、
   スナップショットの `data` サンプル行との**行レベル照合**(4行、丸め差のみ)を実施し、
   「差は画面側のドル単位への丸めだけ」と報告した
4. **不一致の診断が preamble の想定どおり機能した**: 明細行単位の Profitable/Unprofitable
   分類では画面値と一致しないことを検出し、シート側の注文単位 LOD
   (`{ FIXED [Order ID] : SUM([Profit]) }`)を再現して一致させたと明示報告した。
   「不一致はシート計算に起因しうる」という誘導が、原因特定→再現→検証まで到達した例
5. 分析ターン終了後もウィジェットは選択状態のまま健在(この経路では非描画結果の
   再配送が起きなかった。項11 の App 破損は堅牢化修正が別途進行中)
